...
import model_classes.etiss.support
...
def generate_functions(core: model_classes.arch.CoreDef):
    ...
    for fn_name, fn_def in core.functions.items():
        return_type = model_classes.etiss.support.data_type_map[fn_def.data_type]
        if fn_def.size:
            return_type += f'{fn_def.actual_size}'

        context = model_classes.etiss.support.TransformerContext(core.constants, core.address_spaces, core.registers, core.register_files, core.register_aliases, core.memories, core.memory_aliases, fn_def.args, [], core.functions, 0, core_default_width, core_name, True)
        ...
        args_list = [f'{model_classes.etiss.support.data_type_map[arg.data_type]}{arg.actual_size} {arg.name}' for arg in fn_def.args.values()]
    ...
    for enc in reversed(instr_def.encoding):
        if isinstance(enc, model_classes.arch.BitField):
            if enc.name not in seen_fields:
                ...
                fields_code += f'{model_classes.etiss.support.data_type_map[enc.data_type]}{core_default_width} {enc.name} = 0;\n'
...
def generate_instructions(core: model_classes.arch.CoreDef):
    ...
    for (code, mask), instr_def in core.instructions.items():
        ...
        context = model_classes.etiss.support.TransformerContext(core.constants, core.address_spaces, core.registers, core.register_files, core.register_aliases, core.memories, core.memory_aliases, instr_def.fields, instr_def.attributes, core.functions, enc_idx, core_default_width, core_name)
