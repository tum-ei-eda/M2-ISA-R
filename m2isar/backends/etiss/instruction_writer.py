...
def write_functions(core: arch.CoreDef, start_time: str, output_path: pathlib.Path, static_vars: bool, generate_coverage: bool):
	...
	with open(output_path / f'{core_name}Funcs.h', 'w', encoding="utf-8") as funcs_f:
		...
		for fn_name, templ_str in generate_functions(core, static_vars, True, generate_coverage):
	...
	with open(output_path / f'{core_name}Funcs.c', 'w', encoding="utf-8") as funcs_f:
		...
		for fn_name, templ_str in generate_functions(core, static_vars, False, generate_coverage):
			logger.debug("writing function def %s", fn_name)
			funcs_f.write(templ_str)
		funcs_f.write("// clang-format on\n")

def write_instructions(core: arch.CoreDef, start_time: str, output_path: pathlib.Path, separate: bool, static_vars: bool,
	...
	with ExitStack() as stack:
		...
		for instr_name, _, ext_name, templ_str in generate_instructions(core, static_vars, block_end_on, generate_coverage):
