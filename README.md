...
Notes:
- `path/to/input/<top_level>.core_desc` should point to the top-level CoreDSL file (i.e. the file where `Core`s are defined), where `<top_level>` is the name of the top-level file
- `path/to/input/gen_model/<top_level>.m2isarmodel` should point to the internal architecture model file generated by the frontend
